...
// Regiunea efectivă: sursele din Moldova etichetează ele însele știrile
// externe în feed (categoriile „extern", „în lume", „ue/externe").
// Astfel secțiunea Extern există și cu surse exclusiv moldovenești.
function regiuneEfectiva(source, categorii) {
  if (source.region !== 'moldova' || !Array.isArray(categorii)) return source.region;
  const normCat = categorii.map((c) => normalizeText(String(c)));
  const eExterna = normCat.some(
    (c) => c.includes('extern') || c.includes('in lume') || c.includes('international')
  );
  return eExterna ? 'extern' : source.region;
}
...
function normalizeItem(raw, source) {
  ...
  return {
    ...
    region: regiuneEfectiva(source, raw.categories),
    theme,
    ts,
  };
}
...
// Stemare ușoară pentru română: definitive/plurale comune
// („vântul"→„vant", „ploilor"→„ploi", „avertizari"→„avertizar")
function stemRo(cuvant) {
  if (cuvant.length <= 5) return cuvant;
  return cuvant.replace(/(urilor|ului|ilor|urile|itori|atori|arii|ele|uri|ul|ii|lea|le|ea|e|i|a)$/, '');
}

function cuvinteCheie(title) {
  const cuvinte = normalizeText(title)
    .replace(/[^a-z0-9 ]+/g, ' ')
    .split(/\s+/)
  ...
  // bigrame: perechi de cuvinte consecutive — semnal puternic de subiect comun
  const bigrame = new Set();
  for (let i = 0; i < cuvinte.length - 1; i++) bigrame.add(`${cuvinte[i]} ${cuvinte[i + 1]}`);

  return { cuvinte: new Set(cuvinte), bigrame };
}

// Câte cuvinte semnificative din titlul mai scurt apar în cel mai lung,
// plus bonus pentru bigrame comune (ex. „cod galben").
function similaritate(a, b) {
  if (!a.cuvinte.size || !b.cuvinte.size) return 0;
  let inter = 0;
  for (const w of a.cuvinte) if (b.cuvinte.has(w)) inter++;
  let sim = inter / Math.min(a.cuvinte.size, b.cuvinte.size);
  let bigrameComune = 0;
  for (const bg of a.bigrame) if (b.bigrame.has(bg)) bigrameComune++;
  sim += Math.min(0.2, bigrameComune * 0.1);
  return sim;
}
...
function construiesteSubiect(members) {
  ...
  return {
    ...
    // vârsta subiectului = timpul de la PRIMA publicare (cea mai veche sursă
    // din grup) — nu se resetează când alte surse preiau știrea mai târziu
    ts: celMaiVechi.ts,
...
function grupeazaSubiecte(items) {
  ...
  const grupuri = []; // { sets: [{cuvinte, bigrame}], members: [] }

  for (const item of ordonate) {
    const semnatura = cuvinteCheie(item.title);
    let gasit = null;
    // comparăm cu TOȚI membrii grupului, nu doar cu titlul principal:
    // un articol nou poate semăna cu oricare variantă a subiectului
    for (const g of grupuri) {
      if (g.sets.some((s) => similaritate(semnatura, s) >= PRAG_SUBIECT)) {
        gasit = g;
        break;
      }
    }
    if (gasit) {
      gasit.members.push(item);
      gasit.sets.push(semnatura);
    } else {
      grupuri.push({ sets: [semnatura], members: [item] });
    }
  }
