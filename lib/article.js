...
function salveazaInCacheText(url, val) {
  ...
  persistent[url] = { at: Date.now(), val };
  const intrari = Object.entries(persistent);
  if (intrari.length > MAX_TEXT_CACHE) {
    intrari
      .sort((a, b) => a[1].at - b[1].at)
      .slice(0, intrari.length - MAX_TEXT_CACHE)
      .forEach(([k]) => delete persistent[k]);
  }
  try {
    fs.writeFileSync(FISIER_TEXT, JSON.stringify(persistent));
  } catch {
    // serverless: scrierea poate eșua — rămâne cache-ul în memorie
  }
}
