...
function extrageData($) {
  // data publicării are prioritate față de data ultimei modificări
  const publicari = [];
  const modificari = [];
  $('script[type="application/ld+json"]').each((_, el) => {
    try {
      const j = JSON.parse($(el).text());
      cautaCampRecursiv(j, ['datePublished'], publicari);
      cautaCampRecursiv(j, ['dateModified'], modificari);
    } catch {
  ...
  if (metaPub) publicari.push(metaPub);
  for (const g of [...publicari, ...modificari]) {
